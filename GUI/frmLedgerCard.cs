...
namespace GUI
{
    public partial class frmLedgerCard : Telerik.WinControls.UI.RadForm
    {
        DataTable model;
        private string from;
        private string to;
        ...
        public frmLedgerCard()
        {
            ...
            if (Login._bookyear!=null)
                if (Login._bookyear != "")
                year = int.Parse(Login._bookyear);

            dtPeriodFrom.Value = new DateTime(year, 1, 1);
            dtPeriodTo.Value = new DateTime(year, 12, 1);
        }
        ...
        private void Translation()
        {
            using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
            {
                ...
                if (resxSet.GetString(lblPeriodFrom.Text) != null)
                    lblPeriodFrom.Text = resxSet.GetString(lblPeriodFrom.Text);

                if (resxSet.GetString(lblPeriodTo.Text) != null)
                    lblPeriodTo.Text = resxSet.GetString(lblPeriodTo.Text);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            //        to = "999999";
            //    model = new DataTable();
            //    LedgerAccountDAO lad = new LedgerAccountDAO();

            //    model = lad.GetAllAccountsPrint(from, to);

            //    frmLedgerReport frm = new frmLedgerReport(model, name, from, to);
            //    frm.Show();
            //}
            ...
            AccLineDAO ald = new AccLineDAO(Login._bookyear);
                
                dt1 = ald.GetAllAccLineReport(from, to,Login._user.nameUser, dtPeriodFrom.Value, dtPeriodTo.Value);

                frmLedgerCardReport frm = new frmLedgerCardReport(dt1, name, from, to);
                frm.Show();
            //}
