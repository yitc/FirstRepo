...
namespace GUI
{
    ...
    class ReportClassNumberBookings : System.Windows.Forms.UserControl
    {
        string xReport;
        DataTable dt;
        ...
        public void transl(string reportName, DataTable dtReport)
        {
            ...
            rdlcXML.Save("GUI.NumberOptionsPreview.rdlc");
        }
        ...
        private void addTable(XmlDocument rdlcXML)
        {
            ...
            //for (int i = 0; i < 2; i++)
            {
                ...
                using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
                {
                    ...
                    else
                    {
                        if (dt.Columns[i].Caption != null)
                        {
                            ...
                            {
                                wColumn = (decimal)(40) * (decimal)0.0138889;
                            }
                            else if(dt.Columns[i].ColumnName.StartsWith("bookedTravelers"))
                            {
                                wColumn = (decimal)(40) * (decimal)0.0138889;
                            }
                            else if (dt.Columns[i].ColumnName.StartsWith("codeArrangement"))
                            {
                                wColumn = (decimal)(100) * (decimal)0.0138889;
                            }
                            //Za sirinu kolona za engleski jezik ako nije onda za holanski(else)
                            else if (dt.Columns[i].ColumnName.StartsWith("nameArrType"))
                            {
                                ...
                                {
                                    wColumn = (decimal)(this.CreateGraphics().MeasureString(xValue.InnerXml, this.Font).Width + 135) * (decimal)0.0138889;
                                }
                            }
                            //End sirina kolona za engleski jezik
                            else if(dt.Columns[i].ColumnName.StartsWith("nameLabel"))
                            {
                                wColumn = (decimal)(this.CreateGraphics().MeasureString(xValue.InnerXml, this.Font).Width + 25) * (decimal)0.0138889;
                            }
                ...
                xTextBox3.SetAttribute("Name", "Textbox" + (dt.Columns.Count + i + 5).ToString());
                ...
                else if (dt.Columns[i].ColumnName.StartsWith("Column_") || dt.Columns[i].ColumnName.StartsWith("nrTraveler") || dt.Columns[i].ColumnName.StartsWith("bookedTravelers"))
                {
                ...
                xRD3.InnerXml = "Textbox" + (dt.Columns.Count + i + 5).ToString();
