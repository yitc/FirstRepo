...
namespace GUI
{
    ...
    class ReportClassNrPerOfEmployee : System.Windows.Forms.UserControl
    {
        string xReport;
        DataTable dt;
        ...
        public void transl(string reportName, DataTable dtReport)
        {
            ...
            rdlcXML.Save("GUI.ReportNrOfPerEmployee.rdlc");
        }
        ...
        private void addTable(XmlDocument rdlcXML)
        {
            ...
            //for (int i = 0; i < 2; i++)
            {
                ...
                using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
                {
                    ...
                    else if(dt.Columns[i].ColumnName == "Employee1")
                    {
                        wColumn = (decimal)(this.CreateGraphics().MeasureString(xValue.InnerXml, this.Font).Width + 0) * (decimal)0.0;
                    }
                    ...
                    else
                    {
                        if (dt.Columns[i].Caption != null)
                        {
                            ...
                            {
                                wColumn = (decimal)(80) * (decimal)0.0138889;
                            }
                            ...
                            else if (dt.Columns[i].ColumnName.StartsWith("nameArrType"))
                            {
                                ...
                                {
                                    wColumn = (decimal)(this.CreateGraphics().MeasureString(xValue.InnerXml, this.Font).Width + 70) * (decimal)0.0138889;
                                }
                            }
                            else if (dt.Columns[i].ColumnName.StartsWith("date"))
                                wColumn = (decimal)(this.CreateGraphics().MeasureString(xValue.InnerXml, this.Font).Width+10) * (decimal)0.0138889;
                            else
                ...
                xTextBox3.SetAttribute("Name", "Textbox" + (dt.Columns.Count + i + 10).ToString()); //Za kolone sume
                ...
                else if (dt.Columns[i].ColumnName.StartsWith("Column_") || dt.Columns[i].ColumnName.StartsWith("bookedTravelers"))
                {
                ...
                xRD3.InnerXml = "Textbox" + (dt.Columns.Count + i + 10).ToString();//Za kolone suma
            ...
            try
            {
                ...
                if (rdlcXML.GetElementsByTagName("Value") != null)
                    if (rdlcXML.GetElementsByTagName("Value").Count > 0)
                    {
                        ...
                        {
                            if (x.InnerXml == "Date to" || x.InnerXml == "Date from" || x.InnerXml == "Employee")
                            {
                                //Za prevod
                                //
                                using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
