...
namespace GUI
{
    ...
    class ReportClassDepartureList1Ex : System.Windows.Forms.UserControl
    {
        string xReport;
        DataTable dt;
        ...
        public void transl(string reportName, DataTable dtReport)
        {
            ...
            rdlcXML.Save("GUI.ReportDeparturePreview.rdlc");
        }
        ...
        private void addTable(XmlDocument rdlcXML)
        {
            ...
            //for (int i = 0; i < 2; i++)
            {
                ...
                if (dt.Columns[i].ColumnName != "Label")
                {
                    ...
                    using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
                    {
                        ...
                        if (xValue.InnerXml.Contains(" "))
                        {
                            ...
                            wColumn = (decimal)(this.CreateGraphics().MeasureString(LongestWordInArray, this.Font).Width + 33) * (decimal)0.0138889;
                        }
                        ...
                        else if (dt.Columns[i].ColumnName.StartsWith("Type"))
                        {
                            if (Login._user.lngUser == "NL")
                            {
                                wColumn = (decimal)(120) * (decimal)0.0138889;
                            }
                            else
                            {
                        ...
                        else if (dt.Columns[i].ColumnName.StartsWith("Maximum"))
                        {
                            if (Login._user.lngUser == "NL")
                            {
                                wColumn = (decimal)(133) * (decimal)0.0138889;
                            }
                            else
                            {
                                wColumn = (decimal)(105) * (decimal)0.0138889;
                            }
                        }
                        ...
                        {
                            wColumn = (decimal)(this.CreateGraphics().MeasureString(xValue.InnerXml, this.Font).Width + 33) * (decimal)0.0138889;
                        }
                    ...
                    if (i == 0)
                    {
                        ...
                        //Za stavljanje Totala
                    else if (dt.Columns[i].ColumnName == "Maximum" || dt.Columns[i].ColumnName == "TotalBooked")
                    {

                        xValue3.InnerXml = "=sum(Fields!" + dt.Columns[i].ColumnName + ".Value)";
                    }

                        //Za Mnozenje i deljenje vrednosti i formatiranje u dve decimale 
                    else if (dt.Columns[i].Caption == "Occupancy")
                    {

                        xValue3.InnerXml = "=FormatNumber(IIF(sum(Fields!TotalBooked.Value)=0 OR sum(Fields!Maximum.Value) = 0, 0,sum(Fields!TotalBooked.Value)/sum(Fields!Maximum.Value)*100),2)";
                    }
                    //Za Mnozenje i deljenje vrednosti i formatiranje u dve decimale 
