...
namespace GUI
{
    ...
    class ReportClassDepartureList4Exclusive : System.Windows.Forms.UserControl
    {
        string xReport;
        DataTable dt;
        FileStream streamingReport;
        public void transl(string reportName, DataTable dtReport, string label, string type, string btw)
        {
            ...
            try
            {
                ...
                try
                {
                    //Za uzimanje parametara sa forme 
                    ...
                    if (rdlcXML.GetElementsByTagName("PageHeader") != null)
                        if (rdlcXML.GetElementsByTagName("PageHeader").Count > 0)
                        {
                            XmlNode n = rdlcXML.GetElementsByTagName("PageHeader")[0];
                            XmlNode n1 = n.SelectSingleNode("//def:Textbox[@Name='Type']", ns);
                            XmlNode n3 = n.SelectSingleNode("//def:Textbox[@Name='Label']", ns);
                            XmlNode n7 = n.SelectSingleNode("//def:Textbox[@Name='BTW']", ns);
                            if (n1 != null)
                            {
                                XmlNode n2 = n1.ChildNodes[2].ChildNodes[0].ChildNodes[0].ChildNodes[0].ChildNodes[0];
                                n2.InnerXml = type;
                            }
                            if (n3 != null)
                            {
                                XmlNode n5 = n3.ChildNodes[2].ChildNodes[0].ChildNodes[0].ChildNodes[0].ChildNodes[0];
                                n5.InnerXml = label;
                            }

                            if (n7 != null)
                            {
                                XmlNode n6 = n7.ChildNodes[2].ChildNodes[0].ChildNodes[0].ChildNodes[0].ChildNodes[0];
                                n6.InnerXml = btw;
                            }
            ...
            rdlcXML.Save("GUI.ReportDepartureList4ExclusivePreview.rdlc");
        }
        ...
        private void addTable(XmlDocument rdlcXML)
        {
            ...
            //for (int i = 0; i < 2; i++)
            {
                ...
                PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(new DepartureList1Model());
                ...
                using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
                {
                    ...
                    {
                        wColumn = (decimal)(150) * (decimal)0.0138889;
                    }
                    if (dt.Columns[i].ColumnName == "maxTravelers")
                    {
                ...
                xTextBox3.SetAttribute("Name", "Textbox" + (dt.Columns.Count + i + 3).ToString()); //Za kolone sume
                ...
                else if (dt.Columns[i].ColumnName.StartsWith("Column_") || dt.Columns[i].ColumnName.StartsWith("Occupancy") || dt.Columns[i].ColumnName.StartsWith("maxTravelers")
                    || dt.Columns[i].ColumnName.StartsWith("bookedTravelers"))
                {
                ...
                xRD3.InnerXml = "Textbox" + (dt.Columns.Count + i + 3).ToString();//Za kolone suma
            ...
            try
            {
                ...
                if (rdlcXML.GetElementsByTagName("Value") != null)
                    if (rdlcXML.GetElementsByTagName("Value").Count > 0)
                    {
                        ...
                        {
                            if (x.InnerXml == "Date to" || x.InnerXml == "Date from" || x.InnerXml == "Arrangement type" || x.InnerXml == "Label")
                            {
                                //Za prevod
                                //
                                using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
