...
namespace GUI
{
    public partial class frmVoluntaryTrip : frmTemplate
    {
        ...
        private string layoutVolTripAnswer = MainForm.gridFiltersFolder + "\\layoutVolTripAnswer.xml";


        int idCompany;

        public frmVoluntaryTrip()
        {
            InitializeComponent();
        }

        public frmVoluntaryTrip(IModel model)
        {
            InitializeComponent();
        }

        private void frmVoluntaryTrip_Load(object sender, EventArgs e)
        {
            ...
            translation();
            if (File.Exists(layoutVolTripAnswer))
            {
                radGridAnswer.LoadLayout(layoutVolTripAnswer);
            }
                ...
                if (mgvt.CurrentRow.Cells["idAnsType"].Value != null)
                {
                    ...
                    else if (e.Action == Telerik.WinControls.Data.NotifyCollectionChangedAction.Remove)
                    {
                        ...
                        if (mgvt.CurrentRow.Cells["idAns"].Value != null)
                        {
                            ...
                            if (mvb.checkIsOneQuestVT(idGroup) <= 1)
                            {
                                isOneQuest = true;
                            ...
                            if (mvb.checkAnsIsInVTCpr(am.idAns, Convert.ToInt32(mgvt.CurrentRow.Cells["idQueryType"].Value.ToString())) == 0)
                            {
                                if (mvb.checkAnsIsInVTArr(am.idAns, Convert.ToInt32(mgvt.CurrentRow.Cells["idQueryType"].Value.ToString())) == 0)
                                {
                                    if (mvb.DeleteAnsSriptVT(am, isOneAns, isOneQuest, idGroup, this.Name, Login._user.idUser) != true)
                                    {
                                        e.Cancel = true;
            ...
            if (sender.GetType() == typeof(MasterGridViewTemplate))
            {
                ...
                else if (e.Action == Telerik.WinControls.Data.NotifyCollectionChangedAction.Remove)
                {
                    ...
                    if (mgvt.CurrentRow.Cells["idQuest"].Value != null)
                    {
                        ...
                        if (mvb.checkIsOneQuestInGroupVT(qg) <= 1)
                        {
                            isOneInGroup = true;
                        ...
                        if (mvb.GetAnswerVoluntary(qg.idQuest, MainForm.idLabelList).Count == 0)
                        {
                            ...
                            if (mvb.DeleteQuestSriptVT(qg, isOneInGroup, this.Name, Login._user.idUser) != true)
                            {
                                e.Cancel = true;
                                translateRadMessageBox trs = new translateRadMessageBox();
        ...
        private void radMenuButtonSaveLayout_Click(object sender, EventArgs e)
        {
            if (File.Exists(layoutVolTripAnswer))
            {
                File.Delete(layoutVolTripAnswer);
            }
            radGridAnswer.SaveLayout(layoutVolTripAnswer);
