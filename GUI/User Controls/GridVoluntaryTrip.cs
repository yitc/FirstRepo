...
namespace GUI
{    
    public partial class GridViewVoluntaryTrip : System.Windows.Forms.UserControl, IBISGrid
    {
        /// <summary> 
        ...
        public event EventHandler<VoluntaryTripSelectedRowchanged> VoluntaryTripSelectedRowchanged = delegate { };

        public void RaiseStatusChanged(MedicalVoluntaryQuestModel vodd)
        {
            VoluntaryTripSelectedRowchanged(this, new VoluntaryTripSelectedRowchanged { vod = vodd });
        }


        VolontaryTripBUS medBUS;
        ...
        // Folder u kome cuva filtere za voluntary trip
        ...
        public GridViewVoluntaryTrip()
        {
            if (!Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\voluntaryTrip")))
            {
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\voluntaryTrip"));
            }
            if (!Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\voluntaryTrip")))
            {
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\voluntaryTrip"));
            }
            filterFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\voluntaryTrip");
            labelFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\voluntaryTrip");
            medBUS = new VolontaryTripBUS();
        ...
        public void InitializeComponent()
        {
            ...
            // 
            // GridViewVoluntaryTrip
            // 
            this.Controls.Add(this.gridVoluntary);
            this.Name = "GridViewVoluntaryTrip";
        ...
        private void gridVoluntary_CellDoubleClick(object sender, GridViewCellEventArgs e)
        {
            ...
            if (info != null && e.RowIndex >= 0)
            {
                ...
                if (selectedMedical != null)
                {
                    ...
                    frmVoluntaryTrip frm = new frmVoluntaryTrip(this._clickedVoluntary);
        ...
        private void gridVoluntary_KeyDown(object sender, KeyEventArgs e)
        {
            if (this.gridVoluntary.CurrentRow.DataBoundItem != null)
            {
                if (e.KeyData == Keys.Enter)
                {
                    ...
                    frmVoluntaryTrip frm = new frmVoluntaryTrip(model);
                    frm.Show();
                    return;
                }
            }
    ...
    public class VoluntaryTripSelectedRowchanged : EventArgs
    {
