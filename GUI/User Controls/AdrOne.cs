...
{
    public partial class AdrOne : UserControl
    {
        //public AdrOne()
        //{
        ...
        public void ShowHidePostAddress(bool show)
        {
            ...
            if(show == true)
            {
                if(showEmergencyAddress == true)
                    tabAddressContainer.Pages.Insert(3, tabPost);
                else
                    tabAddressContainer.Pages.Insert(2, tabPost);
                
                tabPost.Show();
                tabAddressContainer.SelectedPage = tabPost;
                FillPostAddressAfterCheck();
            }
            else
            {
               
               // tabAddressContainer.Pages.Remove(tabPost); 
                tabPost.Item.Visibility = ElementVisibility.Collapsed;
                tabAddressContainer.SelectedPage = tabAddress;
            }
        }
        public void FillPostAddressAfterCheck()
        {
            ...
            if(pmodel != null)
            {
                if(pmodel.Count > 0)
                {
                    txt_adr_street.Text = pmodel[0].street;
                    txt_adr_houseno.Text = pmodel[0].housenr;
                    txt_adr_zip.Text = pmodel[0].postalCode;
                    txt_adr_ext.Text = pmodel[0].extension;

                    if (pmodel[0].isInternational == true)
                    {
                        rad_adr_inter.CheckState = CheckState.Checked;
                        rad_adr_nl.CheckState = CheckState.Unchecked;

                        btn_adr_get.Visible = false;
                        lbl_adr_country.Visible = true;
                        txt_adr_country.Visible = true;
                        txt_adr_country.Text = pmodel[0].country;
                    }                                       
                }
            }
        }
        public AdrOne(int idContPers)
        {
        ...
        private void ContactPersonAddress_Load(object sender, EventArgs e)
        {
            ...
            if (showBillingAddress == false)
               // tabAddressContainer.Pages.Remove(tabBillingAddress);
                tabBillingAddress.Item.Visibility = ElementVisibility.Collapsed;

            if (showEmergencyAddress == false)
               // tabAddressContainer.Pages.Remove(tabEmergency);
        ...
        private void chk_adr_all_CheckStateChanged(object sender, EventArgs e)
        {
            ...
            if (rck.CheckState == CheckState.Checked)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    txt_adr_country.Text = txt_adr_country.Text;
                    lbl_adr_country.Visible = lbl_adr_country.Visible;
                    txt_adr_country.Visible = txt_adr_country.Visible;


                    rad_badr_nl.CheckState = rad_adr_nl.CheckState;
                    rad_em_nl.CheckState = rad_adr_nl.CheckState;
                    rad_adr_nl.CheckState = rad_adr_nl.CheckState;

                    rad_badr_inter.CheckState = rad_adr_inter.CheckState;
                    rad_em_inter.CheckState = rad_adr_inter.CheckState;
                    rad_adr_inter.CheckState = rad_adr_inter.CheckState;


                    btn_badr_get.Visible = btn_adr_get.Visible;
                    btn_em_get.Visible = btn_adr_get.Visible;
                    btn_adr_get.Visible = btn_adr_get.Visible;
                }
            }
            else
            {
        ...
        private void btn_post_get_Click(object sender, EventArgs e)
        {
            ...
            if (rad_post_nl.IsChecked == true)
            {
                ...
                if (postcode != "" && houseno != "")
                {
                    ...
                    {
                        int digitpost;
                        bool isNumericpost = int.TryParse(postcode.Substring(0, 4), out digitpost);
                        string letterpost = postcode.Substring(4, 2);
                        int indicator = (houseno_num % 2 == 1 ? 0 : 1);

                        if (isNumericpost == true)
                        {
                            DataTable dt = adressBUS.GetAddress(letterpost.ToUpper(), indicator, digitpost, houseno_num);
