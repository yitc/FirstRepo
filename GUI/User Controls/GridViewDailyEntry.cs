...
namespace GUI
{
    ...
    public partial class GridViewDailyEntry : System.Windows.Forms.UserControl, IBISGrid 
    {
        /// <summary> 
        ...
        public event EventHandler<DailyEnStatusSelectedRowchanged> DailyEnStatusSelectedRowchanged = delegate { };
        public void RaiseStatusChanged(AccDailyModel adaily)
        {
            DailyEnStatusSelectedRowchanged(this, new DailyEnStatusSelectedRowchanged { adailye = adaily });
        }
        ...
        public GridViewDailyEntry()
        {
            if (!Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\dailyentry")))
            {
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\dailyentry"));
            }
            if (!Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\dailyentry")))
            {
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\dailyentry"));
            } 
            filterFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username+"\\filters\\custom filters\\dailyentry");
            labelFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\dailyentry");
         ...
         public List<IModel> GetData(int selectedFilter, List<int> idLabelList, string idLang)
        {
           // return  EmployeeBUS.GetAllEmployees(Convert.ToInt32(selectedFilter), idLabelList, idLang); 
            return AccDailyBUS.GetAllDailys();
        }
        ...
        private void gridDaily_DataBindingComplete(object sender, GridViewBindingCompleteEventArgs e)
        {
            ...
            if (gridDaily != null)
            {
                if (gridDaily.ColumnCount > 0)
                {
                    ...
                    gridDaily.Columns["isUseCounter"].IsVisible = false;
                    gridDaily.Columns["inkop"].IsVisible = false;
                    //     gridDaily.Columns["idDailyVerIn"].IsVisible = false;
        ...
        private void gridDaily_CellDoubleClick(object sender, GridViewCellEventArgs e)
        {
            ...
            if (info != null && e.RowIndex >= 0)
                if (selectedDaily != null)
                {
                    ...
                    {
                        frmDailyBankEntry frm = new frmDailyBankEntry(selectedDaily);
                        frm.ShowDialog();
                    ...
                    if (selectedDaily.idDailyType == 2 || selectedDaily.idDailyType == 3)
                    {
                        frmDailyEntry frm = new frmDailyEntry(selectedDaily);
                        //   frmLedgerAccount frm = new frmLedgerAccount();
                        frm.ShowDialog();
                        modelData = AccDailyBUS.GetAllDailys();
        ...
        private void gridDaily_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
        {
            if (this.gridDaily.CurrentRow.DataBoundItem != null)
            {
                if (e.KeyData == Keys.Enter)
                {
                    ...
                    frmDailyEntry frm = new frmDailyEntry(model);
                    frm.Show();
    ...
    public class DailyEnStatusSelectedRowchanged : EventArgs
    {
        public AccDailyModel adailye { get; set; }
