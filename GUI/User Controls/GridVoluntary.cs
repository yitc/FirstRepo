...
namespace GUI
{    
    public partial class GridViewVoluntary : System.Windows.Forms.UserControl, IBISGrid
    {
        /// <summary> 
        ...
        public event EventHandler<VoluntaryStatusSelectedRowchanged> VoluntaryStatusSelectedRowchanged = delegate { };

        public void RaiseStatusChanged(MedicalVoluntaryQuestModel vodd)
        {
            VoluntaryStatusSelectedRowchanged(this, new VoluntaryStatusSelectedRowchanged { vod = vodd });
        }
        ...
        // Folder u kome cuva filtere za voluntary
        ...
        public GridViewVoluntary()
        {
            if (!Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\voluntary")))
            {
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\voluntary"));
            }
            if (!Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\voluntary")))
            {
                Directory.CreateDirectory(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\voluntary"));
            }
            filterFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom filters\\voluntary");
            labelFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\custom labels\\voluntary");
        ...
        public void InitializeComponent()
        {
            ...
            this.gridVoluntary.Text = "Voluntary Grid";
        ...
        private void gridVoluntary_CellDoubleClick(object sender, GridViewCellEventArgs e)
        {
            ...
            if (info != null && e.RowIndex >= 0)
            {
                ...
                if (selectedMedical != null)
                {
                    ...
                    frmVoluntary frm = new frmVoluntary(this._clickedVoluntary);
        ...
        private void gridVoluntary_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
        {
            if (this.gridVoluntary.CurrentRow.DataBoundItem != null)
            {
                if (e.KeyData == Keys.Enter)
                {
                    ...
                    frmVoluntary frm = new frmVoluntary(model);
                    frm.Show();
                    return;
                }
            }
    ...
    public class VoluntaryStatusSelectedRowchanged : EventArgs
    {
