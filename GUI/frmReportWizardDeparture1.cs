...
namespace GUI
{
    public partial class frmReportWizardDeparture1 : RadForm
    {
        //for reports
        DataTable dt72;
        DataTable seldt72;
        ...
        string layoutPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\report filters\\departure list 1");
        ...
        public frmReportWizardDeparture1()
        {
            InitializeComponent();
        ...
        private DataTable DepartureList1()
        {
            ...
            dt = ab.DepartureList1New(model.dateFrom, model.dateTo);
            if (dt != null)
            {
                if (dt.Columns.Count > 0)
                {
                    ...
                    if (dt.Columns["NrType"] != null)
                        dt.Columns["NrType"].Caption = "Nr of trips";
                    ...
                    for (int i = 0; i < dt.Columns.Count; i++)
                    {
                        ...
                        {
                            //za sirinu kolona
                            if (dt.Columns[i].ColumnName == "Type")
                            {
                                wColumn = (decimal)2.5; 
                            }
                            ...
                            else if(dt.Columns[i].ColumnName=="NrType")
                            {
                                wColumn = (decimal)0.6;
                            }
                            else if(dt.Columns[i].ColumnName=="bookedTravelers")
                            {
                                wColumn = (decimal)0.6;
                            }
        ...
        private DataTable DepartureList1Ex()
        {
            model.dateFrom = Convert.ToDateTime(dtFrom.Value);
            model.dateTo = Convert.ToDateTime(dtTo.Value);
            DataTable dt = new DataTable();
            ArrangementBookDAO ab = new ArrangementBookDAO();
            dt = ab.DepartureList1ExNew(model.dateFrom, model.dateTo);
            if (dt != null)
            {
                if (dt.Columns.Count > 0)
                {
