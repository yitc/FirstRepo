namespace GUI
{
    ...
    /// </summary>
    public partial class ReportDepartureList : Telerik.Reporting.Report
    {
        public ReportDepartureList(DataTable dt, Color[] c, Image logo, DateTime dtFrom, DateTime dtTo, BindingList<ReportModel> sel, string typeForm, string labelForm, string btwForm, string nameReport,string themeForm)
        {
            //
            ...
            InitializeComponent();
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    {
                        if (dt.Columns.Contains("bookedTravelers") && dt.Columns.Contains("maxTravelers"))
                            valueTotal = "=Format('{0:N2}',IIF(SUM(Fields.BookedTravelers)=0 OR  SUM(Fields.MaxTravelers)=0,0,cDbl(SUM(Fields.BookedTravelers))/cDbl(SUM(Fields.MaxTravelers))*100))";
                        else
                        {
