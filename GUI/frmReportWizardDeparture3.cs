...
namespace GUI
{
    public partial class frmReportWizardDeparture3 : RadForm
    {
        //for reports
        DataTable dt72;
        DataTable seldt72;
        ...
        string layoutPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Login._user.username + "\\filters\\report filters\\departure list 3");
        ...
        List<ThemeTripModel> listTheme = new List<ThemeTripModel>();
        ...
        public frmReportWizardDeparture3()
        {
            InitializeComponent();
            ...
            ArrangementThemeTripBUS tBus = new ArrangementThemeTripBUS();
            listTheme = tBus.GetAllThemeTripForDepartureList3();

            ddlThemeTrip.DataSource = listTheme;
            ddlThemeTrip.DisplayMember = "nameThemeTrip";
            ddlThemeTrip.ValueMember = "idThemeTrip";
            ddlThemeTrip.SelectedItem.ToString();
        ...
        private void radWizard1_Finish(object sender, EventArgs e)
        {
            ...
            if(seldt72!=null)
                if (seldt72.Rows.Count > 0)
                {
                    ...
                    string theme = ddlThemeTrip.SelectedItem.ToString();
                    string btw = "";


                    ReportViewerDepartureList rvf = new ReportViewerDepartureList(seldt72, c, img, Convert.ToDateTime(dtFrom.Value), Convert.ToDateTime(dtTo.Value), selReportModel, type, label, btw, this.Text.Replace(Login._companyModelList[0].nameCompany + " " + DateTime.Now.Year.ToString() + " - ", ""), theme);
                        rvf.Show();
        ...
        private void radWizard1_Next(object sender, Telerik.WinControls.UI.WizardCancelEventArgs e)
        {
            if (radWizard1.SelectedPage == radWizard1.WelcomePage)
            {
                ...
                //selec theme
                string themeName = ddlThemeTrip.SelectedItem.ToString();

                int indexTheme = ddlThemeTrip.SelectedIndex;
                string themeNameId = ddlThemeTrip.Items[indexTheme].Value.ToString();

                if (themeName != null)
                {
                    idTheme = Convert.ToInt32(themeNameId);
                }

                dt72 = new DataTable();
                if (btnExclusive.IsChecked == true)
                {
                    DepartureList3Ex(idArrType, idLabel,idTheme);
                    dt72 = DepartureList3Ex(idArrType, idLabel,idTheme);

                }
                if (btnInclusive.IsChecked == true)
                {
                    DepartureList3In(idArrType, idLabel,idTheme);
                    dt72 = DepartureList3In(idArrType, idLabel,idTheme);
                }
        ...
        private void setTranslation()
        {
            using (ResXResourceSet resxSet = new ResXResourceSet(Login.resxFile))
            {
                ...
                if (resxSet.GetString(lblThemeTrip.Text) != null)
                    lblThemeTrip.Text = resxSet.GetString(lblThemeTrip.Text);
        ...
        private DataTable DepartureList3In(int idArrType,  int idLabel, int idTheme)
        {
            ...
            dt = dt = ab.DepartureList3New(model.dateFrom, model.dateTo, idArrType, idLabel,idTheme);
            if (dt != null)
            {
                if (dt.Columns.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Columns.Count; i++)
                    {
                        ...
                        if (ddlLayout.SelectedItem==null)
                        {
                            ...
                            else if (dt.Columns[i].ColumnName == "maxTravelers")
                            {
                                wColumn = (decimal)0.6;
                            }
        ...
        private DataTable DepartureList3Ex(int idArrType, int idLabel,int idTheme)
        {
            model.dateFrom = Convert.ToDateTime(dtFrom.Value);
            model.dateTo = Convert.ToDateTime(dtTo.Value);
            DataTable dt = new DataTable();
            ArrangementBookDAO ab = new ArrangementBookDAO();
            dt = ab.DepartureList3ExNew(model.dateFrom, model.dateTo, idArrType, idLabel,idTheme);
            if (dt != null)
            {
                if (dt.Columns.Count > 0)
                {
