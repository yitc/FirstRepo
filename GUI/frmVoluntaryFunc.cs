...
namespace GUI
{
    public partial class frmVoluntaryFunc : frmTemplate
    {
        ...
        private string layoutVolFunct = MainForm.gridFiltersFolder + "\\layoutVolFunct.xml";

        int idCompany;

        public frmVoluntaryFunc()
        {
            InitializeComponent();
        }

        public frmVoluntaryFunc(IModel model)
        {
            InitializeComponent();
        }

        private void frmVoluntaryFunc_Load(object sender, EventArgs e)
        {
            ...
            translation();
            if (File.Exists(layoutVolFunct))
            {
                radGridAnswer.LoadLayout(layoutVolFunct);
            }
                ...
                if (mgvt.CurrentRow.Cells["idAnsType"].Value != null)
                {
                    ...
                    else if (e.Action == Telerik.WinControls.Data.NotifyCollectionChangedAction.Remove)
                    {
                        ...
                        if (mgvt.CurrentRow.Cells["idAns"].Value != null)
                        {
                            ...
                            if (mvb.checkIsOneQuestVF(idGroup) <= 1)
                            {
                                isOneQuest = true;
                        ...
                        //    am.idAns = Convert.ToInt32(mgvt.CurrentRow.Cells["idAns"].Value.ToString());
                            if (mvb.checkAnsIsInVFCpr(am.idAns, Convert.ToInt32(mgvt.CurrentRow.Cells["idQueryType"].Value.ToString())) == 0)
                            {
                                if (mvb.checkAnsIsInVFArr(am.idAns, Convert.ToInt32(mgvt.CurrentRow.Cells["idQueryType"].Value.ToString())) == 0)
                                {
                                    if (mvb.DeleteAnsSriptVF(am, isOneAns, isOneQuest, idGroup, this.Name, Login._user.idUser) != true)
                                    {
                                        e.Cancel = true;
            ...
            if (sender.GetType() == typeof(MasterGridViewTemplate))
            {
                ...
                else if (e.Action == Telerik.WinControls.Data.NotifyCollectionChangedAction.Remove)
                {
                    ...
                    if (mgvt.CurrentRow.Cells["idQuest"].Value != null)
                    {
                        ...
                        if (mvb.checkIsOneQuestInGroupVF(qg) <= 1)
                        {
                            isOneInGroup = true;
                        ...
                        if (mvb.GetAnswerVoluntary(qg.idQuest, MainForm.idLabelList).Count == 0)
                        {
                            ...
                            if (mvb.DeleteQuestSriptVF(qg, isOneInGroup, this.Name, Login._user.idUser) != true)
                            {
                                e.Cancel = true;
                                translateRadMessageBox trs = new translateRadMessageBox();
        ...
        private void radMenuButtonSaveLayout_Click(object sender, EventArgs e)
        {
            if (File.Exists(layoutVolFunct))
            {
                File.Delete(layoutVolFunct);
            }
            radGridAnswer.SaveLayout(layoutVolFunct);
